<script lang="ts">
  ...
  function onClickResize(event: MouseEvent) {
    ...
    startHeight = divElement?.clientHeight;
    startWidth = divElement?.clientWidth;
      ...
      <div>
        ...
        <!-- Open in new page Button-->
         <button on:click={() => window.open(sandboxUrl)} class="px-2 py-1 bg-blue-500 text-white rounded cursor-pointer ml-2">
          <svg xmlns="http://www.w3.org/2000/svg" data-name="Layer 2" viewBox="0 0 24 30" x="0px" y="0px">
            <defs><style>
            .cls-1 {
              stroke-width: 0px;
            }
