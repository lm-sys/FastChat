...
from fastchat.serve.gradio_web_server import (http_bot, set_global_vars,
    get_window_url_params, get_conv_log_filename, block_css,
    build_single_model_ui, no_change_btn, enable_btn, disable_btn,
    get_model_list, load_demo_single)
...
def load_demo_side_by_side(url_params):
    ...
    return (states + dropdown_updates +
...
""")
    ...
    leftvote_btn.click(leftvote_last_response,
        states + model_selectors, [textbox, leftvote_btn, rightvote_btn,
        tie_btn])
...
def build_demo():
    with gr.Blocks(title="Chat with Open Large Language Models",
        ...
        if args.model_list_mode == "once":
            demo.load(load_demo, [url_params], [tabs] + a_list + b_list,
