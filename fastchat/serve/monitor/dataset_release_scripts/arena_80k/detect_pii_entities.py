...
df = pd.read_json('./fastchat/serve/monitor/dataset_release_scripts/arena_80k/clean_battle_conv_20240222_80k.json')
old_df_len = len(df)
...
# Define your columns
columns = ["redacted_conversation_a", "entities_a", "redacted_conversation_b", "entities_b", "unknown_language"]
...
with open('redacted_conversations-80k.csv', 'w', newline='', encoding='utf-8') as file:
    ...
    for i, row in df.iterrows():
        csv_row = {}
        try:
            if row['language'] in language_codes.keys() and language_codes[row['language']] != "n/a":
