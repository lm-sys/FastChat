...
class State:
    def __init__(self, model_name):
        ...
        self.session_deadline = time.time() + SESSION_EXPIRATION_TIME
    ...
    def session_expired(self) -> bool:
        return self.session_deadline < time.time()
...
def regenerate(state: State, request: gr.Request):
...
def clear_history(state: State, request: gr.Request):
    logger.info(f"clear_history. ip: {request.client.host}")
    state.reset_conv()
    return (state, [], "") + (disable_btn,) * 5


def add_text(state: State, model_selector, text, request: gr.Request):
    ip = request.client.host
    logger.info(f"add_text. ip: {ip}. len: {len(text)}")

    state.update_model(model_selector)
...
"""
    ...
    model_selector.change(clear_history, state, [state, chatbot, textbox] + btn_list)
