...
def build_about():
    about_markdown = """
# About Us
**Software Arena** ([replace with link here](https://link)) is an open platform for comparing and evaluating AI-generated programs with visual user interfaces through human preference. Built by a team of researchers from [Organization A], [Organization B], [Organization C], and [Organization D], Software Arena enables users to interact with programs generated by large language models (LLMs) and vote on the quality of their user interfaces. The platform builds upon the open-source [FastChat project](https://github.com/lm-sys/FastChat) by introducing features like interactive sandboxes, allowing users to explore programs with visual UIs. Additionally, we are actively collecting datasets from user interactions and plan to open-source them in the future to support further research and development. 
