...
class LMDeployWorker(BaseModelWorker):
    ...
    async def generate_stream(self, params):
        ...
        results_generator = InterFace.engine.generate(context, int(request_id),
        ...
        async for request_output in results_generator:
            ...
            ret = {
                ...
                'usage': {
                    ...
                    'total_tokens': request_output.input_token_len+request_output.generate_token_len,
                },
            ...
            yield (
                json.dumps({**ret, 'finish_reason': request_output.finish_reason}, ensure_ascii=False)
                + '\0'
            ).encode('utf-8')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--quant-policy',
                                   type=int,
                                   default=0,
                                   help='Whether to use kv int8. When k/v is '
                                   'quantized into 8 bit, set it to 4')
    ...
    backend_config=TurbomindEngineConfig('base',
                                         model_format=args.model_format,
                                         tp=args.tp, session_len=args.session_len,
                                         max_batch_size=args.max_batch_size,
                                         cache_block_seq_len=args.cache_block_seq_len,
                                         quant_policy=args.quant_policy,
                                         rope_scaling_factor=args.rope_scaling_factor,
                                         use_logn_attn=args.use_logn_attn)
    InterFace.engine = pipeline(args.model_path, backend_config=backend_config, log_level=args.log_level)
