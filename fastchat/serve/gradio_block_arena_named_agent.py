...
from fastchat.serve.gradio_web_server_agent import (
    State,
    bot_response,
...
def add_text(
    ...
    for i in range(num_sides):
        ...
        states[i].system_conv.append_message(states[i].system_conv.roles[0], text)
        states[i].system_conv.append_message(states[i].system_conv.roles[1], None)
