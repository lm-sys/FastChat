...
Alternatively use can use Azure OpenAI endpoint. In this case you need to set AZURE_OPENAI_ENDPOINT and AZURE_OPENAI_KEY environment variables and also pass in deployment name via --azure-deployment-name argument:

```
export AZURE_OPENAI_ENDPOINT=https://xyz.openai.azure.com/
export AZURE_OPENAI_KEY=XXXXXX
python gen_judgment.py --model-list [LIST-OF-MODEL-ID] --parallel [num-concurrent-api-call] --azure-deployment-name [deplyoment-name-from-azure]
```
...
python -m fastchat.serve.vllm_worker --model-path [MODEL-PATH]
