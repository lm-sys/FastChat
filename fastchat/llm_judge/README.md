...
A standalone script is also available at `misc/polar_plot.py`. It relies on a configuration filename (for which an example can be found in `./misc/polar_plot_configs/gpt4-judge-all.json`) in which the model judgment file and target models can be changed. To run it:
...
Other examples can be found in `./misc/polar_plot_configs/` and a bash script to run all of them is available at `./misc/polar_plot.sh`.
