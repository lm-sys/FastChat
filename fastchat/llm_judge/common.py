...
if LooseVersion(openai.__version__) > LooseVersion("1.0"):
    ...
    print(f"WARNING: Openai version {openai.__version__} is deprecated please use version >= 1.0")
import anthropic
...
def chat_completion_openai(model, conv, temperature, max_tokens, api_dict=None):
    if LooseVersion(openai.__version__) < LooseVersion("1.0"):
        return chat_completion_openai_depricated(model, conv, temperature, max_tokens, api_dict=api_dict)
    if api_dict is None:
        client = OpenAI()
    else:
...
def chat_completion_openai_depricated(model, conv, temperature, max_tokens, api_dict=None):
...
def chat_completion_openai_azure(model, conv, temperature, max_tokens, api_dict=None):
    if LooseVersion(openai.__version__) < LooseVersion("1.0"):
        return chat_completion_openai_azure_depricated(model, conv, temperature, max_tokens, api_dict=api_dict)
    if api_dict is not None:
...
def chat_completion_openai_azure_depricated(model, conv, temperature, max_tokens, api_dict=None):
