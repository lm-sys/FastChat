...
def should_process_file(card_id, file_index, total_cards=8):
    total_datasets = 31
...
def load_questions(question_file: str, begin: Optional[int], end: Optional[int], tokenizer, model_id="baichuan-chat"):
    ...
    all_questions = []
    ques_file = json.load(open(question_file,encoding="utf-8"))
    for topic in ques_file:
        for id in topic["questions"]:
            ques=topic["questions"][id]
            all_questions.append({"turns": ["请阅读题干，从以下选项中选出你认为正确的一项："+ques], "category": "QA", "question_id": id})
