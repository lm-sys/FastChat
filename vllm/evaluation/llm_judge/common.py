...
import sys
sys.path.append('/home/Userlist/madehua/code/fc/fastchat')
import openai
import anthropic
...
def load_questions(question_file: str, begin: Optional[int], end: Optional[int]):
    ...
    questions = []
    with open(question_file, "r", encoding="utf-8") as ques_file:
        file_qs = json.load(ques_file)
        for file_q in file_qs:
            for k,v in file_q["questions"].items():
                question = {"question_id":k ,"category": "QA", "turns":[v]}
...
def chat_compeletion_openai(model, conv, temperature, max_tokens):
    ...
    for _ in range(API_MAX_RETRY):
        try:
            ...
            with open(api_key_path,mode="r",encoding="utf-8") as fr:
                openai.api_key = json.loads(fr.readline())
