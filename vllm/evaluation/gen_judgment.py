...
def make_match_single(
    ...
    for q in questions:
        ...
        for i in range(len(models)):
            q_id = q["question_id"]
            m = models[i]
            if type(next(iter(model_answers[m])))==str:
                a = model_answers[m][q_id]
            else:
                a = model_answers[m][int(q_id)]
            if ref_answers is not None:
                ref = ref_answers[judge.model_name][q_id]
                matches.append(
                    MatchSingle(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--begin", type=int, default=None, help="the begin of questions"
    )
    parser.add_argument(
        "--end", type=int, default=None, help="the end of questions"
    )
    args = parser.parse_args()
    api_key_path = "key.json"
    with open(api_key_path,mode="w",encoding="utf-8") as fw:
        api_key = json.dumps(args.api_key,ensure_ascii=False)
        fw.write(api_key)
    question_file = f"/home/Userlist/madehua/code/vllm/{args.question_file_name}/questions.json"
    answer_dir = f"/home/Userlist/madehua/code/vllm/output/model_answer"
    ref_answer_dir = f"/home/Userlist/madehua/code/vllm/output/reference_answer"

    # Load questions
    questions = load_questions(question_file, args.begin, args.end)
    ...
    if args.mode == "single":
        ...
        output_file = (
            f"/home/Userlist/madehua/code/vllm/output/judge_result/{args.bench_name}/{args.judge_model}_single.jsonl"
        )
        # output_file = (
        ...
        output_file = (
            f"/home/Userlist/madehua/code/vllm/output/judge_result/{args.bench_name}//{args.judge_model}_pair.jsonl"
        )
    ...
    for match in matches:
        ...
        uniq_id = f"{match.question['question_id']}_{match.model}_{match.judge.model_name}_{match.judge.prompt_template['name']}_{turn}"
