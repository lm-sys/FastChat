...
class SupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        def should_read(file_name):
            if not data_cate:
                return True
            if "_" in data_cate:
                keyword, exclude = data_cate.split("_")
                return keyword in file_name and exclude not in file_name
            else:
                return data_cate in file_name

        for file in data_files:
            if should_read(file.name):
                with open(file, 'r') as f:
                    self.data.extend([json.loads(line) for line in f])



        print(f"Total number of {data_cate if data_cate else ''} data entries read: {len(self.data)}")
...
def train():
    ...
    dataset = SupervisedDataset(
        data_args.data_dir, data_args.data_cate, tokenizer, training_args.model_max_length
    )
